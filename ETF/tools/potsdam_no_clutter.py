...
def clip_big_image(image_path, clip_save_dir, clip_size=512, stride_size=256, to_label=False):
    ...
    xmin = x * stride_size #NOTE: The original setting was clip_size here, but which is logical error. It ignores the stride_size and thus crop:512, stride:512 is generated.
    ymin = y * stride_size #NOTE: The original setting was clip_size here, but which is logical error. It ignores the stride_size and thus crop:512, stride:512 is generated.
    xmin = xmin.ravel()
    ymin = ymin.ravel()
...
def main():
    ...
    for zipp in zipp_list:
        with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
            ...
            prog_bar = tqdm.tqdm(range(len(src_path_list)))
            for i, src_path in enumerate(src_path_list):
                idx_i, idx_j = osp.basename(src_path).split('_')[2:4]
                data_type = 'train' if f'{idx_i}_{idx_j}' in splits['train'] else 'val'
                clip_size, stride_size = (args.clip_size, args.stride_size) if data_type == 'train' else (args.clip_size, args.clip_size)
